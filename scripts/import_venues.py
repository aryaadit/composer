...
def process_venue(
    ...
    venue_id = trim(raw.get("venue_id", ""))
    if not venue_id:
        name = trim(raw.get("name", ""))
        if name:
            errors.append(f"{name}: missing venue_id — skipped")
            stats["skipped_no_venue_id"] += 1
        return None
...
-- Idempotent upsert via venue_id (UNIQUE constraint).
...
"""
    ...
    for v in venues:
        values_lines.append(
            f"-- {v['venue_id']} — {v['name']} ({v['neighborhood']}, tier {v['price_tier']})\n"
...
SELECT venue_id, name FROM composer_venues ORDER BY venue_id LIMIT 10;
"""
...
def main() -> int:
    ...
    # Dedupe on venue_id — can't INSERT two rows with the same key.
    # Last row wins (in case the sheet has accidental duplicates).
    seen: dict[str, int] = {}
    deduped: list[dict] = []
    for v in venues:
        key = v["venue_id"]
    ...
    print(f"Skipped (no venue_id): {stats['skipped_no_venue_id']:>4}", file=sys.stderr)
